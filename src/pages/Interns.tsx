...
export default function InternsPage() {
  ...
  const filtered = useMemo(() => {
    return interns.filter(i => {
      const matchesSearch = i.fullName.toLowerCase().includes(search.toLowerCase()) || i.internId.toLowerCase().includes(search.toLowerCase()) || i.department.toLowerCase().includes(search.toLowerCase());
  ...
  const exportCSV = () => {
    ...
    const rows = interns.map(i => `${i.internId},${i.fullName},${i.school},${i.course},${i.department},${i.status},${i.startDate},${i.endDate}`).join('\n');
    ...
    return (
      <DashboardLayout>
        <div className="max-w-3xl">
          <h1 className="text-2xl font-display font-bold text-foreground mb-6">{editingIntern ? 'Edit Intern' : 'Add New Intern'}</h1>
    ...
    <DashboardLayout>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setShowForm(true)} className="gradient-brand text-primary-foreground gap-2 hover:opacity-90"><Plus size={16} /> Add Intern</Button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search by name, ID, or department..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10" />
        </div>
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)} className="rounded-lg border border-input bg-background px-3 py-2 text-sm">
      ...
      <div className="glass-card rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading ? (
                 <tr><td colSpan={7} className="text-center py-12 text-muted-foreground"><Loader2 className="animate-spin mx-auto mb-2" size={24} />Loading interns from database...</td></tr>
              ) : filtered.length > 0 ? (
                filtered.map(intern => (
                  <tr key={intern.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                    <td className="py-3 px-4">
                      {intern.photo ? <img src={intern.photo} alt="" className="w-10 h-10 rounded-full object-cover border-2 border-brand-orange/30" /> : <UserCircle size={40} className="text-muted-foreground" />}
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${intern.status === 'Active' ? 'bg-success/10 text-success' : 'bg-muted text-muted-foreground'}`}>{intern.status}</span>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex gap-1">
                        <button onClick={() => setEditingIntern(intern)} className="p-2 rounded-lg hover:bg-muted transition-colors text-muted-foreground hover:text-foreground"><Pencil size={14} /></button>
                        <button onClick={() => handleDelete(intern.id)} className="p-2 rounded-lg hover:bg-destructive/10 transition-colors text-muted-foreground hover:text-destructive"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))
