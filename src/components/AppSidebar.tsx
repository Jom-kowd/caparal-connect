    ...
    <aside className="w-72 bg-sidebar border-r border-border h-screen flex flex-col hidden lg:flex sticky top-0">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        <div>
          ...
          <div className="space-y-1"><Link to="/dashboard" className={linkClass('/dashboard')}><LayoutDashboard size={18} /> Dashboard</Link><Link to="/scan" className={linkClass('/scan')}><ScanLine size={18} /> Universal QR Scanner</Link></div>
        </div>
        <div>
          <p className="px-4 text-xs font-bold uppercase tracking-wider text-sidebar-foreground/40 mb-2">Interns</p>
          <div className="space-y-1"><Link to="/interns" className={linkClass('/interns')}><Users size={18} /> Intern Directory</Link><Link to="/attendance" className={linkClass('/attendance')}><CalendarDays size={18} /> Attendance Logs</Link><Link to="/id-cards" className={linkClass('/id-cards')}><IdCard size={18} /> Generate Intern IDs</Link></div>
        </div>
        <div>
          <p className="px-4 text-xs font-bold uppercase tracking-wider text-sidebar-foreground/40 mb-2">Employees</p>
          <div className="space-y-1"><Link to="/employees" className={linkClass('/employees')}><Briefcase size={18} /> Employee Directory</Link><Link to="/employee-attendance" className={linkClass('/employee-attendance')}><CalendarDays size={18} /> Employee Logs</Link><Link to="/employee-id-cards" className={linkClass('/employee-id-cards')}><IdCard size={18} /> Generate Employee IDs</Link></div>
        </div>
      </div>
